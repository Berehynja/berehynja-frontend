...
export function ScheduleCalendar({ programs, ageGroups, onLoadingChange }: ScheduleCalendarProps) {
  ...
  return (
    <div className="font-nunito">
      ...
      {/* Сітка колонок для кожної вікової групи (однакові колонки) */}
      <div className="grid gap-6 xl:grid-cols-3">
        {eventsByAge.map(({ groupId, groupName, lessons }) => {
          // Групуємо заняття за часовими слотами
          const timeSlots = lessons.reduce(
            (acc, lesson) => {
              const slot = `${lesson.timeStart}-${lesson.timeEnd}`;
              if (!acc[slot]) acc[slot] = [];
          ...
          return (
            <div key={groupId} className="rounded-2xl bg-gray-50 p-6 shadow-lg xl:col-span-1">
              <h2 className="text-preset-3 text-Blue mb-4 text-center font-bold">
                {t("programs.kids.groups")} {groupName}
              </h2>

              {/* Проста таблична сітка: 2 колонки для часу і 4 для назви заняття */}
                  ...
                  sortedSlots.map((slot) => {
                    ...
                    return (
                      <React.Fragment key={slot}>
                        ...
                        {/* Контейнер для занять у цей слот */}
                        <div className="col-span-4 grid grid-cols-1 gap-2">
                          {slotLessons.map((lesson) => {
                            ...
                            return (
                              <div
                                key={lesson.id}
                                className="shadow-card group relative col-span-full flex items-center justify-between rounded-md p-3 transition-all"
                                style={{ backgroundColor: bgColor }}
                              >
                                <div className="flex w-full flex-col items-center justify-center text-center">
                                  <span className="text-preset-4 font-nunito leading-tight font-bold">
                                  ...
                                  {/* Рівень / Примітки (де тепер можна писати підгрупу) */}
                                  {lesson.level && (
                                    <span className="text-preset-5 text-gray-800">
                                      ({lesson.level})
                                    </span>
