  ...
  return (
    <>
      <div className="mb-12 text-center">
        <h2 className="font-nunito mb-4 flex items-center justify-center gap-3 text-3xl">
      ...
      <div className="grid grid-cols-1 gap-10 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {team.map((member) => (
          ...
          <div key={member.id} className="group relative">
            <MemberCard member={member} setSelectedMember={setViewingMember} />
            ...
            {isAdmin && (
              <button
                ...
                className="absolute top-4 right-4 z-10 cursor-pointer rounded-full border border-slate-100 bg-white/90 p-3 text-slate-600 shadow-lg backdrop-blur-sm transition-all duration-300 ease-in-out hover:bg-blue-600 hover:text-white"
        ...
        {isAdmin && (
          <div
            ...
            className="group flex min-h-[400px] cursor-pointer flex-col items-center justify-center rounded-[3rem] border-2 border-dashed border-slate-200 bg-slate-50/50 text-slate-400 transition-all duration-500 ease-out hover:-translate-y-2 hover:border-blue-400 hover:bg-blue-50/50 hover:text-blue-500"
          >
            <div className="mb-4 rounded-full bg-white p-5 shadow-sm transition-all duration-300 group-hover:scale-110 group-hover:shadow-md">
              <Plus size={32} />
            </div>
            <span className="font-nunito text-xs tracking-[0.2em] uppercase">Додати фахівця</span>
          </div>
        )}
      </div>
